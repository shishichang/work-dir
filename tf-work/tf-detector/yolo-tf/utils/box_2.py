...
def box_constructor(meta, net_out_in):
    ...
    classed_num_filtered = classes_num_filtered[argsort]
    ...
    for i in range(len(probs_filtered)):
        ...
        for j in range(i+1, len(bboxes_filtered)):
            ...
            #if iou(bboxes_filtered[i], bboxes_filtered[j] > 0.4):
            if box_iou(a, b) > 0.4:
                probs_filtered[j] = 0
    ...
    for i in range(len(bboxes_filtered)):
        ...
        result['x2'] = bboxes_filtered[i][0] + bboxes_filtered[i][2] 
        result['y2'] = bboxes_filtered[i][1] + bboxes_filtered[i][3]
