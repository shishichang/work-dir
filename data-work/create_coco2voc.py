...
with open(coco_voc_map, 'r') as f1:
    for line in f1.readlines():
        line_strs = line.split(',')
        coco_label = line_strs[0]
        coco_voc[coco_label] = line_strs[1]
print(coco_voc)
...
for i, filename in enumerate(find_files(anno_in_dir)):
    ...
    with open(anno_file, 'r') as f_in:
        ...
        for i, v in enumerate(info['annotation']):
            ...
            if coco_voc.has_key(str(label)):
                print(label, coco_voc[str(label)], "in VOC")
                #with open(anno_out_file, 'a') as f_out:
                #    f_out.write("%s %d %d %d %d\n"% (coco_voc[str(label)], xmin, ymin, xmax, ymax))
            else:
                print(label, classes[label], 'is not in VOC')
