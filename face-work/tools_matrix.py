...
def filter_face_48net(cls_prob,roi,pts,rectangles,width,height,threshold):
    ...
    print("pts:")
    print(pts[pick,:])
    print("w,h,x1,y1,x2,y2:%s,%s,%s,%s,%s,%s"%(w,h,x1,y1,x2,y2))
    print(pts[pick,:])
    if h.shape[1] == 0:
        rectangles = []
        return rectangles  
    ...
    if is_mine == True:
        w = x2 - x1
        h = y2 - y1
        print('W:%s H:%s w:%s h:%s'%(width,height,w,h))
        pts0= np.array([(w*pts[pick,0])[0]]).T + x1
        pts1= np.array([(h*pts[pick,5])[0]]).T + y1
        pts2= np.array([(w*pts[pick,1])[0]]).T + x1
        pts3= np.array([(h*pts[pick,6])[0]]).T + y1
        pts4= np.array([(w*pts[pick,2])[0]]).T + x1
        pts5= np.array([(h*pts[pick,7])[0]]).T + y1
        pts6= np.array([(w*pts[pick,3])[0]]).T + x1
        pts7= np.array([(h*pts[pick,8])[0]]).T + y1
        pts8= np.array([(w*pts[pick,4])[0]]).T + x1
        pts9= np.array([(h*pts[pick,9])[0]]).T + y1
